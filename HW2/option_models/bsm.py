...
class BsmModel:
    ...
    def delta(self, strike, spot, texp, cp_sign=1):
        ...
        d1=np.log(spot/strike)/(self.vol*np.sqrt(texp))+1/2*self.vol*np.sqrt(texp)
        return cp_sign*np.exp(-self.divr*texp)*ss.norm.cdf(d1*cp_sign)

    def vega(self, strike, spot, vol, texp, cp_sign=1):
        d1=np.log(spot/strike)/(self.vol*np.sqrt(texp))+1/2*self.vol*np.sqrt(texp)
        return np.exp(-self.divr*texp)*spot*np.sqrt(texp)*ss.norm.pdf(d1)

    def gamma(self, strike, spot, vol, texp, cp_sign=1):
        d1=np.log(spot/strike)/(self.vol*np.sqrt(texp))+1/2*self.vol*np.sqrt(texp)
        return np.exp(-self.divr*texp)*ss.norm.pdf(d1)/(spot*self.vol*np.sqrt(texp))
